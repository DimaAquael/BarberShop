...
function RSGoPro_OnOfferChangeDetail($elementObj) {
  ...
  if (finedOfferID > 0) {
    ...
    $elementObj
      .find(".changeimage.imgofferid" + finedOfferID)
      .show()
      .addClass("scrollitem");
    $elementObj
      .find(".changeimage.imgofferid" + finedOfferID)
...
function RSGoPro_FancyImagesOnUpdate() {
  setTimeout(function () {
    ...
    $(".fancygallery")
      .find(".image .max")
      .css("maxHeight", parseInt($(".fancygallery").parents(".fancybox-inner").height()) - 5);
    $(".fancygallery")
      .find(".slider .max")
      .css("height", parseInt($(".fancygallery").parents(".fancybox-inner").height()) - 5 - 60);
  }, 50);
}
...
$(document).ready(function () {
  ...
  $(window).on("hashchange", function () {
    if (RSGoPro_AfterLoading) {
      $(".detailtabs.tabs")
        .find('.switcher[href="' + window.location.hash + '"]')
        .trigger("click");
    }
  });
  // tabs -> add review
  ...
  $(document).on("RSGoProOnFancyBeforeShow", function () {
    if (RSGoPro_DetailBuy1Click) {
      var value = "";
      value =
        BX.message("RSGoPro_DETAIL_PROD_ID") +
        ": " +
        $(".elementdetail").find(".js-add2basketpid").val() +
        "\n" +
        BX.message("RSGoPro_DETAIL_PROD_NAME") +
        ": " +
        $(".elementdetail").data("elementname") +
        "\n" +
        BX.message("RSGoPro_DETAIL_PROD_LINK") +
        ": " +
  ...
  $(document).on("RSGoProOnFancyBeforeShow", function () {
    if (RSGoPro_DetailCheaper) {
      var value = "";
      value =
        BX.message("RSGoPro_DETAIL_CHEAPER_TITLE") +
        "\n" +
        +"\n" +
        BX.message("RSGoPro_DETAIL_PROD_ID") +
        ": " +
  ...
  /*sn*/
  try {
    if ($(".b1c-wrapper").length) {
      $.ajax({
        type: "GET",
        url: "/include/buy1click.php",
        success: function (response) {
          $(".b1c-wrapper").append($(response));
          value =
            BX.message("RSGoPro_DETAIL_PROD_ID") +
            ": " +
            $(".elementdetail").find(".js-add2basketpid").val() +
            ...
            "\n" +
            "-----------------------------------------------------";
          $(".b1c-wrapper").find('textarea[name="RS_AUTHOR_ORDER_LIST"]').text(value);
        },
        error: function () {},
      });
    }
